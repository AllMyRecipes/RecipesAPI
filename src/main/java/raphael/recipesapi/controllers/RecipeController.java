...
public class RecipeController {
    ...
    public ResponseEntity<Page<Recipe>> getRecipeByIngredient(@RequestParam("ingredientId")Long ingredientId, @RequestParam("page")int page){
        System.out.println("Ingredient ID: " + ingredientId);
        Page<Recipe> listRecipes = null;
        try {
            PageRequest pageRequest = PageRequest.of(page, 5);
            listRecipes = recipeService.getRecipesByIngredientId(pageRequest, ingredientId);
        }catch (Exception e){
            log.info(e.getMessage());
        }
        return ResponseEntity.ok().body(listRecipes);
